...
export default function LoginPage() {
    ...
    const t = {
        welcome: lang === 'th' ? 'ยินดีต้อนรับกลับ' : 'Welcome Back',
        subtitle: lang === 'th' ? 'เข้าสู่ระบบเพื่อจัดการร้านค้าของคุณ' : 'Sign in to manage your shop',
        email: lang === 'th' ? 'อีเมล' : 'Email',
        password: lang === 'th' ? 'รหัสผ่าน' : 'Password',
        rememberMe: lang === 'th' ? 'จดจำฉัน' : 'Remember me',
        forgotPassword: lang === 'th' ? 'ลืมรหัสผ่าน?' : 'Forgot password?',
        login: lang === 'th' ? 'เข้าสู่ระบบ' : 'Sign In',
        noAccount: lang === 'th' ? 'ยังไม่มีบัญชี?' : "Don't have an account?",
        register: lang === 'th' ? 'สมัครสมาชิก' : 'Sign Up',
        errInvalidCredential: lang === 'th' ? 'อีเมลหรือรหัสผ่านไม่ถูกต้อง' : 'Invalid email or password',
        errInvalidEmail: lang === 'th' ? 'รูปแบบอีเมลไม่ถูกต้อง' : 'Invalid email format',
        errDisabled: lang === 'th' ? 'บัญชีนี้ถูกระงับการใช้งาน' : 'This account has been disabled',
        errGeneric: lang === 'th' ? 'เกิดข้อผิดพลาด กรุณาลองใหม่อีกครั้ง' : 'An error occurred. Please try again',
        orContinueWith: lang === 'th' ? 'หรือเข้าสู่ระบบด้วย' : 'Or continue with',
        continueGoogle: lang === 'th' ? 'Google' : 'Google',
        continueFacebook: lang === 'th' ? 'Facebook' : 'Facebook',
        continueLine: lang === 'th' ? 'LINE' : 'LINE',
        socialError: lang === 'th' ? 'ไม่สามารถเข้าสู่ระบบได้ กรุณาลองใหม่' : 'Unable to sign in. Please try again',
    }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (err.code === 'auth/user-not-found' || err.code === 'auth/wrong-password' || err.code === 'auth/invalid-credential') {
                setError(t.errInvalidCredential)
            } else if (err.code === 'auth/invalid-email') {
                setError(t.errInvalidEmail)
            } else if (err.code === 'auth/user-disabled') {
                setError(t.errDisabled)
            } else {
                setError(t.errGeneric + ' (' + err.code + ')')
            }
        } finally {
            setIsLoading(false)
        }
        ...
        <div className="min-h-screen pt-20 pb-12 flex flex-col items-center justify-center bg-gray-50 dark:bg-bg-dark">
            <div className="w-full max-w-md bg-white dark:bg-surface-dark p-8 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800">
                <div className="text-center mb-8">
                    <h1 className="text-3xl font-display font-bold mb-2">{t.welcome}</h1>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex items-center justify-between text-sm">
                        <label className="flex items-center space-x-2 cursor-pointer">
                            ...
                            <span className="text-text-secondary dark:text-gray-400">{t.rememberMe}</span>
                        </label>
