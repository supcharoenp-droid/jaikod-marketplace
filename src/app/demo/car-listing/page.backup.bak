    ...
    sections: [
        {
            ...
            fields: [
                ...
                {
                    key: 'body_type', label_th: 'ประเภทตัวถัง', label_en: 'Body Type', importance: 'required', type: 'select',
                    options: [
        ...
        },
        {
            ...
            fields: [
                ...
                {
                    key: 'owners', label_th: 'เจ้าของกี่มือ', label_en: 'Owners', importance: 'required', type: 'select',
                    options: [
        ...
        },
        {
            ...
            fields: [
                {
                    key: 'fuel_type', label_th: 'เชื้อเพลิง', label_en: 'Fuel', importance: 'required', type: 'select',
                    options: [
        ...
        },
        {
            ...
            fields: [
                {
                    key: 'accident_history', label_th: 'ประวัติอุบัติเหตุ', label_en: 'Accident', importance: 'required', type: 'select',
                    options: [
                ...
                {
                    key: 'flood_history', label_th: 'ประวัติน้ำท่วม', label_en: 'Flood', importance: 'required', type: 'select',
                    options: [
        ...
        },
        {
            ...
            fields: [
                ...
                {
                    key: 'tax_status', label_th: 'ภาษี/พ.ร.บ.', label_en: 'Tax Status', importance: 'required', type: 'select',
                    options: [
                ...
                {
                    key: 'book_status', label_th: 'สมุดทะเบียน', label_en: 'Book', importance: 'required', type: 'select',
                    options: [
        ...
        },
        {
            ...
            fields: [
                // Thai Location Picker (special type - renders ThaiLocationPicker component)
                {
                    ...
                    importance: 'required', type: 'location_picker'
                },
                // Landmark (full width)
                {
...
function CarListingDemo() {
    ...
    const [publishSuccess, setPublishSuccess] = useState<{ listing_number: string; slug: string } | null>(null)
    ...
    const generateTitle = useMemo(() => {
        const { brand, model, sub_model, year } = formData
        if (!brand || !model) return t.emptyTitle
        const parts = [brand, model]
        // Skip sub_model if it's 'null' or empty
        ...
        if (year) {
            ...
            if (yearOption) parts.push(language === 'th' ? yearOption.label_th : yearOption.label_en)
        }
        return parts.join(' ')
                    ...
                    <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-8 max-w-md w-full text-center border border-white/10 shadow-2xl animate-fade-in">
                        ...
                        <div className="bg-slate-900/50 rounded-xl p-4 mb-6 border border-purple-500/30">
                            <div className="text-sm text-gray-400 mb-1">
                                {language === 'th' ? 'เลขประกาศ' : 'Listing Number'}
                            </div>
                            <div className="text-2xl font-bold text-purple-400 font-mono">
                                {publishSuccess.listing_number}
                            </div>
                        </div>
                ...
                <div className="grid md:grid-cols-5 gap-4">
                    ...
                    <div className={`${activeTab === 'form' ? 'block' : 'hidden'} md:block md:col-span-3`}>
                        <div className="bg-slate-800/50 backdrop-blur-lg rounded-2xl border border-white/10 overflow-hidden">
                            ...
                            {/* Form Sections - ขยายความสูงให้เลื่อนน้อยลง */}
                                        ...
                                        if (fieldInlineGroup) {
                                            if (fieldInlineGroup !== currentGroup) {
                                                if (currentGroupFields.length > 0) {
                                                    fieldGroups.push({ group: currentGroup, fields: currentGroupFields })
                                                }
                                                currentGroup = fieldInlineGroup
                                                currentGroupFields = [field]
                                            } else {
                                            ...
                                            fieldGroups.push({ group: null, fields: [field] })
                                        }
                                    })
                                    if (currentGroupFields.length > 0) {
                                        fieldGroups.push({ group: currentGroup, fields: currentGroupFields })
                        ...
                        <div className="sticky top-24">
                            <div className="bg-slate-800/50 backdrop-blur-lg rounded-2xl border border-white/10 overflow-hidden">
                                ...
                                <div className="p-5">
                                    ...
                                    {/* Title & Price */}
                                    <div className="mb-4">
                                        <h3 className="text-xl font-bold text-white mb-2">{generateTitle}</h3>
